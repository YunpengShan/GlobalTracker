...
class LoginViewController: UIViewController {
    ...
    @IBOutlet weak var showHidePasswordButton: UIButton! // Added button for showing/hiding password
    
    let userDefaults = UserDefaults.standard
    
    var isPasswordVisible = false // Track if password is visible
    ...
    override func viewDidLoad() {
        ...
        if let rememberedEmail = userDefaults.string(forKey: "rememberedEmail") {
        ...
        // Add action for show/hide password button
        showHidePasswordButton.addTarget(self, action: #selector(showHidePasswordTapped(_:)), for: .touchUpInside)
    }
    ...
    @objc func rememberMeSwitchChanged(_ sender: UISwitch) {
        ...
        // Clear stored email if "Remember Me" is disabled
        if !sender.isOn {
            userDefaults.removeObject(forKey: "rememberedEmail")
        }
    }
    
    @objc func showHidePasswordTapped(_ sender: UIButton) {
        // Toggle password visibility
        passwordTextField.isSecureTextEntry = !isPasswordVisible
        isPasswordVisible = !isPasswordVisible
        
        // Change button image based on password visibility
        let imageName = isPasswordVisible ? "Group 1.1.2" : "Group 1.2.1"
        showHidePasswordButton.setImage(UIImage(named: imageName), for: .normal)
    }
